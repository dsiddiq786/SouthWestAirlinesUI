...
export default function ReturnDatesBar() {
  const { returnDate } = useFlights();

  // Ensure returnDate is a valid Date object
  const validDepartDate = isValid(returnDate) ? returnDate : new Date(); // Fallback to today's date if returnDate is invalid

  const [selectedDate, setSelectedDate] = useState(validDepartDate); // Track selected date

  // Generate an array of the next 4 dates (including returnDate)
