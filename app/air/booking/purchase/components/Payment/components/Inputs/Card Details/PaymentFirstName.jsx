    ...
    <div className="flex flex-col">
      ...
      <div className="relative">
        <input
          ...
          {...register(`payment.firstName`, {
            required: 'Enter first name',
          })}
          onChange={() => clearErrors(`payment.firstName`)} // Clear error on input change
          className={`h-[32px] w-[214px] rounded-sm py-[3px] pl-[7px] text-[13px] ${
            errors?.payment?.firstName
      ...
      <span className="h-4 text-sm">
        {errors?.payment?.firstName && (
          <span className="text-[11px] text-red-600">
            {errors.payment.firstName.message?.toString()}
          </span>
        )}
      </span>
    </div>
