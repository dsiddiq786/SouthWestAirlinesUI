    ...
    <div className="flex flex-col">
      ...
      <div className="relative">
        <input
          ...
          {...register(`payment.billingInfo.cityTown`, {
            required: 'Enter city/town',
          })}
          onChange={() => clearErrors(`payment.billingInfo.cityTown`)} // Clear error on input change
          className={`h-[32px] w-[329px] rounded-sm py-[3px] pl-[7px] text-[13px] ${
            errors?.payment?.billingInfo?.cityTown
      ...
      <span className="h-4 text-sm">
        {errors?.payment?.billingInfo?.cityTown && (
          <span className="text-[11px] text-red-600">
            {errors.payment.billingInfo.cityTown.message?.toString()}
          </span>
        )}
      </span>
    </div>
