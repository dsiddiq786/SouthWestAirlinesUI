...
import ArriveStates from './ArriveStates';
...
export default function Arrival() {
  ...
  useEffect(() => {
    setIsArriveEmpty(!arriveSearchQuery.length > 0 && isSearchClicked);
  }, [arriveSearchQuery]);
  ...
  return (
    <div ref={dropdownRef} className="relative flex flex-col">
      ...
      <div className="relative">
        <input
          className={`w-[184px] rounded-sm py-[2px] pl-[7px] pr-[48px] text-[32px] font-bold leading-none ${isArriveEmpty ? 'border border-red-600' : 'inner-box-shadow-sw border'} text-blue-sw shadow-inner`}
          onClick={() => {
            setIsDropDownOpen(true);
          }}
          type="text"
          value={arriveSearchQuery}
          onChange={(e) => handleArriveSearch(e.target.value)}
        />
        {isArriveEmpty && (
        ...
        {isArriveEmpty ? (
          ...
          <>
            {selectedArriveCodes.length > 1 ? (
              <span className="text-[11px] text-gray-sw">{`${filteredArriveCitiesByState[0].state} Airports`}</span>
            ) : (
              <>
      ...
      {isDropDownOpen && arriveSearchQuery.length >= 2 && (
        <>
          <div className="absolute -left-5 bottom-[5.7rem] z-50 w-[200%] rounded-sm border bg-white shadow-lg shadow-gray-400 xl:w-[240%]">
            <ul className="-mb-3 max-h-64 w-full overflow-y-scroll px-3 pt-3">
              {filteredArriveCitiesByState.length > 0 ? (
                filteredArriveCitiesByState.map((state) => (
                  <div key={uuidv4()}>
                    <ArriveStates state={state} />
                  </div>
                ))
              ) : (
